...
func TestCheckHDate(t *testing.T) {
	...
	if rh == rhConverted {
		t.Log(
			"This version of hdate produces struct-equal results from .New() and .FromTime(). However, we should not be depending on private fields, so xhdate.Equal (via CheckHDate) is still useful.",
		)
	}
